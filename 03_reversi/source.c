...
bool CheckCanPlace(int _color,             // 石の色
                   VEC2 _position,         // 座標
...
bool CheckCanPlaceAll(int _color) {
  ...
  for (int y = 0; y < BOARD_HEIGHT; y++) {
    ...
    for (int x = 0; x < BOARD_WIDTH; x++) {
      ...
      if (CheckCanPlace(_color,    // int _color       石の色
...
void DrawScreen() {
  ...
  if (isPlayer[turn]) {
    ...
    for (int x = 0; x < BOARD_WIDTH; x++) {
      ...
      if (x == cursorPosition.x) {
        printw("↑"); // [6-5-12]↑矢印を表示する
      } else {
        printw("　"); // [6-5-13]全角スペースを表示する
      }
    }
  }
  ...
  if (turn != TURN_NONE) {
    // [6-5-16]ターンを表示する
    printw("%sのターンです¥n", turnNames[turn]);
  }
  // [6-5-17]決着が付いたなら
  else {
    // [6-5-18]黒い石の数を宣言する
    ...
    // [6-5-29]引き分けかどうか判定する
    if (winner == TURN_NONE) {
      printw("引き分け¥n"); // [6-5-30]引き分けメッセージを表示する
    } else                  // [6-5-31]優劣が付いたなら
    {
      // [6-5-32]勝者を表示する
      printw("%sの勝ち¥n", turnNames[winner]);
    }
  }
}
...
void SelectMode() {
  ...
  while (1) {
    ...
    // [6-6-4]メッセージを表示する
    printw("モードを　選択して¥nください¥n");

    printw("¥n¥n"); // [6-6-5]2行空ける
    ...
    for (int i = 0; i < MODE_MAX; i++) {
      ...
      printw("%s¥n", modeNames[i]); // [6-6-8]モードの名前を描画する

      printw("¥n"); // [6-6-9]1行空ける
    }
